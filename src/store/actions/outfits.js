import { setOutfitApi } from '../../apis/apis';
...
export const setOutfit = data => {
    return (dispatch) => {
        setOutfitApi(data).then(
            res => dispatch({type: SET_OUTFIT_SUCCESS, payload: res}),
...
export const deleteOutfitFromList = data => {
    return { type: DELETE_OUTFIT_FROM_LIST, payload: data }
}

export const getOutfitList = data => {
    return (dispatch) => {
        dispatch({ type: GET_OUTFIT_LIST, payload: data })
    }
}